...
export function initBot(username) {
    ...
    // Fix T2-#6: Retry mechanism for bot.chat() to handle transient failures
        ...
        const send = () => {
            try {
                ...
                if (attempts < maxAttempts) {
                    attempts++;
                    console.warn(`[bot.chat] Failed to send message, retrying (${attempts}/${maxAttempts})...`);
                    setTimeout(send, 1000 * attempts);
                } else {
                    console.error(`[bot.chat] Failed to send message after ${maxAttempts} attempts:`, err);
                }
            }
        };
        send();
...
export function getFoodPoints(itemName) {
    if (!mcdata || !mcdata.foodsByName) return null;
    const food = mcdata.foodsByName[itemName];
    return food ? food.foodPoints : null;
}
