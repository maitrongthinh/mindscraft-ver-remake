...
export function proceduralGeneration(m = 20,
    ...
    const MAX_TOTAL_ATTEMPTS = rooms * 200; // Fix T2-#2
    let totalAttempts = 0;

    while (placedRooms < rooms && totalAttempts < MAX_TOTAL_ATTEMPTS) {
        totalAttempts++;
        if (bot.interrupt_code) break;
...
function matrixToBlueprint(matrix, startCoord) {
    ...
    // Fix T2-#11: Explicitly clear the large input matrix to assist GC
