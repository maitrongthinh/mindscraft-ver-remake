...
export class CookingTaskInitiator {
    ...
    async init() {
        ...
        await this.safeChat(`/fill ~ ~-1 ~ ~50 ~-3 ~50 grass_block`);
        await this.safeChat(`/fill ~ ~-1 ~ ~-50 ~-3 ~50 grass_block`);
        await this.safeChat(`/fill ~ ~-1 ~ ~-50 ~-3 ~-50 grass_block`);
        await this.safeChat(`/fill ~ ~-1 ~ ~50 ~-3 ~-50 grass_block`);
        await this.safeChat(`/fill ~ ~ ~ ~50 ~10 ~50 air`);
        await this.safeChat(`/fill ~ ~ ~ ~-50 ~10 ~50 air`);
        await this.safeChat(`/fill ~ ~ ~ ~-50 ~10 ~-50 air`);
        await this.safeChat(`/fill ~ ~ ~ ~50 ~10 ~-50 air`);
        ...
        try {
            for (let i = 0; i < 6; i++) {
                for (let j = 0; j < 6; j++) {
                    ...
                    const z = zStart + j;
                    if (till) {
                        await this.safeChat(`/setblock ${x} ${position.y - 1} ${z} farmland`);
                    }
                    await this.safeChat(`/setblock ${x} ${position.y} ${z} ${crop_and_age}`);
                }
            }
            await new Promise(resolve => setTimeout(resolve, 300));
        } catch (err) {
    ...
    async plantSugarCane (patches) {
        ...
        for (const patch of patches) {
            ...
            await this.safeChat(`/setblock ${xCenter} ${position.y - 1} ${zCenter} water`);
            const offsets = [[1, 0], [-1, 0], [0, 1], [0, -1]];
            for (const [dx, dz] of offsets) {
                await this.safeChat(`/setblock ${xCenter + dx} ${position.y} ${zCenter + dz} sugar_cane[age=15]`);
            }
        }
    };
    ...
    async plantMushrooms(xStart, zStart) {
        ...
        for (let i = 0; i < 4; i++) {
            for (let j = 0; j < 5; j++) {
                const x = xStart + i;
                const z = zStart + j;
                await this.safeChat(`/setblock ${x} ${position.y - 1} ${z} mycelium`);
                const mushroomType = (i + j) % 2 === 0 ? 'red_mushroom' : 'brown_mushroom';
                await this.safeChat(`/setblock ${x} ${position.y} ${z} ${mushroomType}`);
            }
        }
    }
    ...
    async summonAnimals (animals, amount) {
        ...
        for (const animal of animals) {
            for (let i = 0; i < amount; i++) {
                ...
                await this.safeChat(`/summon ${animal} ${Math.floor(x)} ${position.y} ${Math.floor(z)}`);
            }
        }
    }
    ...
    async killEntities(entities) {
        for (const entity of entities) {
            await this.safeChat(`/kill @e[type=${entity},distance=..200]`);
        }
    }
    ...
    async buildHouse (xStart, zStart) {
                ...
                for (let z = startZ; z <= startZ + width; z++) {
                    if (y === startY) {
                        if (!(x === startX + depth - 1 && z === startZ + Math.floor(width / 2))) {
                            await this.safeChat(`/setblock ${x} ${y} ${z} stone_bricks`);
                        }
                        continue;
                    }
        ...
        await this.safeChat(`/setblock ${startX + depth - 1} ${startY} ${doorZ} stone_brick_stairs[facing=west]`);
        await this.safeChat(`/setblock ${startX + depth} ${startY} ${doorZ} air`);
            ...
            for (let x = startX + i; x <= startX + depth - i; x++) {
                for (let z = startZ + i; z <= startZ + width - i; z++) {
                    if (x === startX + i || x === startX + depth - i ||
                        ...
                        await this.safeChat(`/setblock ${x} ${startY + height + i} ${z} cobblestone`);
                    }
                }
            }
        }
        ...
        // Interior items
        await this.safeChat(`/setblock ${startX + 4} ${startY + 1} ${startZ + 3} crafting_table`);
        await this.safeChat(`/setblock ${startX + 4} ${startY + 1} ${startZ + 5} furnace`);
        // Add fuel to the furnace
        await this.safeChat(`/data merge block ${startX + 4} ${startY + 1} ${startZ + 5} {Items:[{Slot:1b,id:"minecraft:coal",Count:64b}]}`)
        await this.safeChat(`/setblock ${startX + 4} ${startY + 1} ${startZ + 7} smoker`);
        // Add fuel to the smoker
        await this.safeChat(`/data merge block ${startX + 4} ${startY + 1} ${startZ + 7} {Items:[{Slot:1b,id:"minecraft:coal",Count:64b}]}`)
        await this.safeChat(`/setblock ${startX + depth - 3} ${startY + 1} ${startZ + 2} bed`);
        await new Promise(resolve => setTimeout(resolve, 300));
    }

    async safeChat(message, maxRetries = 3) {
        let lastError;
        for (let i = 0; i < maxRetries; i++) {
            try {
                ...
                lastError = err;
                console.warn(`[CookingTask] chat attempt ${i + 1} failed: ${err.message}`);
                await new Promise(resolve => setTimeout(resolve, 1000 * Math.pow(2, i)));
            }
        }
        throw new Error(`[CookingTask] chat failed after ${maxRetries} attempts: ${lastError.message}`);
    }
}