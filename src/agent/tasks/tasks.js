...
const hellsKitchenProgressManager = {
    ...
    writeProgress: async function (progress) {
        ...
        while (_progressFileLock) {
            if (this.agent && this.agent.bot && this.agent.bot.interrupt_code) break;
...
export class Task {
    ...
    async initBotTask() {
        try {
            ...
            console.error(`[Task] Failed to initialize bot task: ${error.message}`);
            await this._rollbackInitialization();
            throw error;
        }
    }

    async _rollbackInitialization() {
        console.log('[Task] Rolling back initialization changes...');
        try {
            await this.agent.bot.chat(`/clear ${this.name}`);
            // Force reset any stuck states
            this.agent.bot.pathfinder.stop();
            if (this.agent.bot.digging) this.agent.bot.stopDigging();
        } catch (err) {
