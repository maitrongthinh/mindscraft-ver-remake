...
export class NPCContoller {
    ...
    async executeGoal() {
        // Safety first: retreat from nearby hostile mobs before pursuing any goal
        const hostile = world.getNearestEntityWhere(this.agent.bot, entity => mc.isHostile(entity), 8);
        if (hostile) {
            await this.agent.actions.runAction('npc:escapeHostile', async () => {
                await skills.avoidEnemies(this.agent.bot, 16);
            });
            return;
        }
