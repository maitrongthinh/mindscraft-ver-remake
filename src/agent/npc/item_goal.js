...
export class ItemGoal {
    ...
    async executeNext(item_name, item_quantity = 1) {
        ...
        // Before gathering/crafting, check if a nearby chest already has what we need
        const inv_count = world.getInventoryCounts(this.agent.bot)[next.name] || 0;
        if (inv_count < quantity) {
            const chest = world.getNearestBlock(this.agent.bot, 'chest', 16);
            if (chest) {
                const needed = quantity - inv_count;
                await this.agent.actions.runAction('itemGoal:checkChest', async () => {
                    await skills.takeFromChest(this.agent.bot, next.name, needed);
                });
                // Re-check inventory after chest attempt
                const after_chest = world.getInventoryCounts(this.agent.bot)[next.name] || 0;
                if (after_chest >= quantity) {
                    console.log(`Found ${next.name} in nearby chest for goal ${this.goal.name}`);
                    return true;
                }
            }
        }
