...
// WeakMap to cache Movements instances per bot to prevent memory leaks (T1-#3)
const movementsCache = new WeakMap();

function getMovements(bot) {
    if (!movementsCache.has(bot)) {
        const move = new pf.Movements(bot);
        movementsCache.set(bot, move);
    }
    return movementsCache.get(bot);
}
...
export async function wait(bot, milliseconds) {
    ...
    while (timeLeft > 0 && !bot.interrupt_code) {
...
export async function smeltItem(bot, itemName, num = 1) {
    ...
    while (total < num && !bot.interrupt_code) {
...
export async function attackEntity(bot, entity, kill = true) {
    ...
    if (!kill) {
        ...
        while (world.getNearbyEntities(bot, 24).includes(entity) && !bot.interrupt_code) {
            await new Promise(resolve => setTimeout(resolve, 1000));
            if (bot.interrupt_code) {
                bot.pvp.stop();
                return false;
...
export async function goToGoal(bot, goal) {
    ...
    try {
        const timeout = 60000; // 60s timeout for pathfinding
        const start = Date.now();

        const pathfindPromise = bot.pathfinder.goto(goal);
        const timeoutPromise = new Promise((_, reject) => {
            const timer = setInterval(() => {
                if (bot.interrupt_code) {
                    clearInterval(timer);
                    bot.pathfinder.stop();
                    reject(new Error('Pathfinding interrupted'));
                }
            ...
            pathfindPromise.finally(() => clearInterval(timer));
        });
        ...
        clearInterval(doorCheckInterval);
        if (err.message === 'Pathfinding interrupted' || err.message === 'Pathfinding timeout') {
            log(bot, err.message);
            return false;
        }
...
function startDoorInterval(bot) {
    ...
    const doorCheckInterval = setInterval(() => {
        ...
        if (stuck_time > 1200) {
            ...
            while (currentIndex != 0 && !bot.interrupt_code) {
...
export async function useDoor(bot, door_pos = null) {
    ...
    await new Promise((resolve) => setTimeout(resolve, 1000));
    while (bot.pathfinder.isMoving() && !bot.interrupt_code) {
        if (bot.interrupt_code) break;
...
export async function goToBed(bot) {
    ...
    bot.modes.pause('unstuck');
    while (bot.isSleeping && !bot.interrupt_code) {
        if (bot.interrupt_code) break;
        await new Promise(resolve => setTimeout(resolve, 500));
    }
