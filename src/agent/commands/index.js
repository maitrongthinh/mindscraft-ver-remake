...
function normalizeCommonName(arg, paramType) {
    ...
    const normalized = arg.trim().toLowerCase().replaceAll(' ', '_');
    const sharedAliases = {
        wood: 'oak_log',
        tree: 'oak_log',
        logs: 'oak_log',
    ...
    if ((paramType === 'BlockName' || paramType === 'BlockOrItemName' || paramType === 'ItemName') && sharedAliases[normalized]) {
        return sharedAliases[normalized];
    }
    return normalized;
}
