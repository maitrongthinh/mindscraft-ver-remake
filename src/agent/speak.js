...
async function processQueue() {
    ...
    if (model === 'system') {
        // system TTS — sanitize text to prevent command injection
        const safeTxt = txt.replace(/[^a-zA-Z0-9 .,!?'\-]/g, '');
        const cmd = isWin
            ...
            $s.Speak('${safeTxt.replace(/'/g, "''")}'); $s.Dispose()"`
            : isMac
                ? `say "${safeTxt.replace(/"/g, '\\"')}"`
                : `espeak "${safeTxt.replace(/"/g, '\\"')}"`;
